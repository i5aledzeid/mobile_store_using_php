...
Class Master extends DBConnection {
	...
	function save_clients(){
		...
		$check = $this->conn->query("SELECT * FROM `clients` where `firstname` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function delete_clients(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Clients successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
